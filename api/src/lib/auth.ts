...
import { apiKey, magicLink, oAuthProxy, twoFactor, organization } from 'better-auth/plugins';
...
export function getAuth(env : Env) {
  const db = new Kysely<Database>({
      dialect: new LibsqlDialect({
          url: env.DATABASE_URL || 'libsql://local.db',
    ...
    plugins: [
      ...
      magicLink({
        sendMagicLink: async ({ email, token, url }, request) => {
            const appBase = import.meta.env.AUTH_BASE_URL || process.env.AUTH_BASE_URL || 'http://localhost:4321'.replace(/\/+$/,'');
      ...
      passkey({
        rpID: env.PASSKEY_RP_ID || (() => {
          try { return new URL(env.AUTH_BASE_URL || 'http://localhost:4321').hostname; } catch { return 'localhost'; }
        })(),
        rpName: env.PASSKEY_RP_NAME || 'CapySchool',
        origin: env.AUTH_BASE_URL || 'http://localhost:4321'.replace(/\/+$/,''),
      }),
      apiKey(),
      organization(),
      oAuthProxy({ 
