      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data && data.user) {
            ...
            if (appCfg && savedRedirect && isRedirectAllowed(appCfg, savedRedirect)) {
    ...
    <div className="min-h-screen flex items-center justify-center p-3">
      <div className="w-full max-w-lg bg-gray-800/50 backdrop-blur-lg rounded-2xl shadow-xl p-6 border border-gray-700">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-400">Sign in to your account using your preferred method</p>
          {/* App info */}
          {app && errors.length === 0 && (
            <div className="mt-2">
              <p className="text-xs text-gray-300">Sign in to <span className="font-semibold">{app.name}</span></p>
              <p className="text-[11px] text-gray-400">{app.description}</p>
              {redirectUrl && (() => { try { const o = new URL(redirectUrl).origin; return (<p className="text-[11px] text-gray-500">Destination: {o}</p>);} catch { return null; } })()}
            </div>
          )}
        </div>
        ...
        {/* OAuth Providers */}
        <div className="space-y-2.5">
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">OAuth Providers</h2>
